...
echo -e "${YELLOW}Step 1/4: Starting Image Server (port 8000)${NC}"
start_service "image-server" "python3 image_server.py" "8000"
sleep 2

echo -e "\n${YELLOW}Step 2/4: Starting Search Service (port 8006)${NC}"
cd search-service
if [ ! -d ".venv" ]; then
...
echo -e "\n${YELLOW}Step 3/4: Starting Onboarding Service (port 8005)${NC}"
...
echo -e "\n${YELLOW}Step 4/4: Starting Widget Dev Server (port 5173)${NC}"

...
This guide explains how to maintain the agent-doc system in this repo and how to reuse the same standard in future projects.
...
5. Put active work only in `memory.md`, and keep it under the stated size limit.
6. Put partial-session context in `handoff.md` only when another agent must continue unfinished work.
7. If two files say the same thing, delete the duplicate and keep one owner.
...
- Update `COLLABORATIVE.md` when the collaboration model itself changes.

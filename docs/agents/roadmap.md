...
| **[Latency] Re-test `turn_eagerness`/`speculative_turn` now that per-turn tracking exists** | Agent | ⬜ Pending — **blocked on deploy** | 1–2 hrs | Was tried live before and reverted for a real regression — see `[[elevenlabs-latency-reverts]]` memory: `eager`+`speculative_turn=true` caused premature interruptions (reverted 2026-04-10). Don't reapply on faith — bump `LATENCY_CONFIG_VERSION`, ship to a small traffic slice, and check `/api/latency-summary` + a live listening/interruption check before rolling out further. Dropped `optimize_streaming_latency` from this item: it's a deprecated field as of ElevenLabs' Feb 2026 global-routing changeover, and was separately reverted 2026-07-04 for audible TTS dropouts — not worth re-testing. |

...
| # | Task | Status | Notes |
|---|------|--------|-------|
| 1 | Create `agent_requests` table in Supabase | ⬜ **Not done on the live pilot project** | 2026-08-12: confirmed via the Supabase REST schema (`jchigqerypjwmszslzke` — the project both `onboarding-service/.env` and `search-service/.env` point at) that `agent_requests` does not exist; only `products`, `session_feedback`, `turn_latency`, `search_latency`. This was marked done for an earlier/different Supabase project. **Impact:** every admin route that reads/writes `agent_requests` (`/api/requests`, `/api/requests/{id}`, `/api/process`, the whole client-acquisition intake flow in `routes/client.py`) will fail on this project until the table is created — not just `/api/latency-summary` (which was patched 2026-08-12 to degrade gracefully instead of 500ing). Not urgent for the xfused pilot itself (its agent was created directly, not via the request form), but blocks onboarding any *next* client through the public form. |
...
|------|-------|--------|--------|-------|
| **⭐ [Post-pilot #1] Search-result cache (subset of Refactor A)** | Agent | ✅ Deployed 2026-09-06 | 30 min | 5-min TTL FIFO cache in `search-service/main.py`. Live box @ `3f411ce` has `SEARCH_CACHE_ENABLED=true` and `SEARCH_CONFIG_VERSION=v3-error-persist`. Cache hits were already visible in 2026-09-04 `tp-search` logs; new rows after this restart are tagged `v3-error-persist`. |
...
| Add search-service result cache (LRU+TTL) | Agent | ✅ Done (2026-07-20) | 30 min | Same cache as Post-pilot #1 above (duplicate line — consolidate next cleanup pass). |
...
| Voice-test Hindi/Tamil pronunciation quality on `eleven_flash_v2` (not the multilingual model) | Human | ✅ Resolved (2026-08-12) | — | Answered live, not in code: the pilot agent was hand-switched in the ElevenLabs dashboard to `agent.language="hi"` (base language, not just a preset) which unlocks `tts.model_id=eleven_flash_v2_5` — that's the multilingual model this task was asking whether to add. It's been running in production since (10 real conversations through 2026-08-06). See [[decisions.md 2026-08-12]] entry. `create_agent()` now takes a `language` param so this is reproducible for future agents. Telugu still has no supported real-time voice model. |
...
| **[Latency] Re-test `turn_eagerness`/`speculative_turn` now that per-turn tracking exists** | Agent | ⬜ Pending — **blocked on A1–A10 numbers** | 1–2 hrs | Code is live as of 2026-09-06 (`LATENCY_CONFIG_VERSION=v3-heardyou-searchfail`). Was tried live before and reverted for premature interruptions (`eager`+`speculative_turn=true`, 2026-04-10). Do not reapply until `/api/latency-summary` + checklist A1–A10 say this is the bottleneck. Dropped `optimize_streaming_latency`: deprecated as of ElevenLabs' Feb 2026 global-routing changeover; separately reverted 2026-07-04 for audible TTS dropouts. |
| **[Latency] Trim `PROMPT_CLAUDE` system prompt** | Agent | ⏭️ Deferred (2026-07-20) | 1–2 hrs | ~3,055 words / 20× "This step is important" (`elevenlabs_agent.py:302-443`). Measured, then deferred: most of the length is very recent deliberate reinforcement for real bugs (transition-narration ban was the commit immediately prior to this measurement) — expected savings (~100-200ms across 2 LLM calls/turn) didn't justify the regression risk to tool-call reliability mid-pilot. Revisit only with a plan to thoroughly re-test tool-chain/narration/language-switch behavior before any rollout. |
| **[Latency] Move webhook/search-service closer to ElevenLabs' region** | Agent + Human | ⬜ Pending — **premise likely stale, measure first** | 4–8 hrs | 2026-08-12: ElevenLabs made global routing the default in Feb 2026 (server picked by geographic proximity), so the "US/EU only" assumption behind this task may no longer hold for India traffic. ElevenLabs also now offers India data residency (`api.in.residency.elevenlabs.io`) but it's Enterprise-only. Do NOT move search-service to the US preemptively — that would break colocation with Supabase (`ap-south-1`) and Lightsail (`ap-south-1a`), trading one ocean crossing for another. Get real `/api/latency-summary` numbers first; only pursue this (or ask ElevenLabs sales about Asia routing/India residency) if the webhook round-trip is still the measured bottleneck. |
...
| Date | Task | Who |
|------|------|-----|
| 2026-09-06 | Lightsail deploy of SEARCH_FAIL / v3 latency tags (`3f411ce`); live widget copied; Wrina `language=hi` unchanged. A1–A10 still to run. | Cloud agent |

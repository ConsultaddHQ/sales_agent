...
|------|-------|--------|--------|-------|
| **⭐ [Post-pilot #1] Search-result cache (subset of Refactor A)** | Agent | ✅ Done (2026-07-20) | 30 min | Shipped as a 5-min TTL FIFO cache keyed on `(store_id, normalized_query)` in `search-service/main.py` (`SEARCH_CACHE_ENABLED`/`SEARCH_CACHE_TTL_SECONDS`/`SEARCH_CACHE_MAX_ENTRIES` env vars). Cache hits are tagged (`cache_hit=true`) in the new `search_latency` table so hit-rate and its latency win are directly measurable via `/api/latency-summary`. |
...
| **[Latency] Re-test `turn_eagerness`/`speculative_turn`/`optimize_streaming_latency` now that per-turn tracking exists** | Agent | ⬜ Pending | 1–2 hrs | 2026-07-20: per-turn latency tracking (`turn_latency`/`search_latency` + `/api/latency-summary`, see decisions.md) now makes it possible to A/B these safely. Both were tried live before and reverted for real regressions — see `[[elevenlabs-latency-reverts]]` memory: `eager`+`speculative_turn=true` caused premature interruptions (reverted 2026-04-10); `optimize_streaming_latency=3` caused audible TTS dropouts (reverted 2026-07-04). Don't reapply on faith — bump `LATENCY_CONFIG_VERSION`, ship to a small traffic slice, and check `/api/latency-summary` + a live listening/interruption check before rolling out further. |
...
| **[Latency] Move webhook/search-service closer to ElevenLabs' region** | Agent + Human | ⬜ Pending | 4–8 hrs | ElevenLabs Conversational AI runs out of US/EU, not India; the `search_products`/`get_product_details` webhook round-trips Mumbai↔US/EU at least once per turn (~150-200ms each way). Investigate whether a lightweight US-region proxy (or the full search-service) is worth the added Supabase-Mumbai cross-region hop it would introduce — net win only if the ElevenLabs↔webhook savings exceed the new US↔Supabase cost. Needs real measurement via `/api/latency-summary` before committing to a region move. |

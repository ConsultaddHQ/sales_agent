...
| Verify + wire ElevenLabs language-detection system tool + language_presets | Human/Agent | ⬜ Pending | 1 hr | 2026-07-03: shipped multilingual TTS (`eleven_flash_v2_5`) + a prompt directive so the agent detects Hindi/Tamil and replies in-language — this works today via the LLM + TTS alone. NOT shipped: `conversation_config.language_presets` and the dedicated "language detection" system tool that gives ElevenLabs-level STT-side auto-switching — ElevenLabs docs pages 404'd on fetch during implementation, so the exact JSON schema is unverified and guessing risked a rejected PATCH on the live client agent. Verify the schema (dashboard UI or a working docs URL) and wire it for better Hindi/Tamil transcription accuracy. Telugu is not supported on `eleven_flash_v2_5` (only the slower Eleven v3) — offer only after ElevenLabs ships it on a low-latency model. |

...
const ERROR_MESSAGES = {
  invalid_url: {
    title: "Invalid URL",
    message:
      "Please enter a valid URL (e.g., example.myshopify.com or https://example.com)",
  },
  not_shopify_store: {
    title: "Not a Shopify Store",
    message:
      "This doesn't appear to be a Shopify store. Please enter a valid Shopify store URL.",
  },
  password_protected: {
    title: "Store is Password Protected",
    message:
      "This store is password-protected. Please disable the password in your Shopify settings and try again.",
    help: "Go to Shopify Admin → Online Store → Preferences → Password Protection",
  },
  no_products_found: {
    title: "No Products Found",
    message:
      "No products found in this store. Please add and publish products first.",
  },
  rate_limited: {
    title: "Rate Limited",
    message:
      "Shopify is rate-limiting requests. Please try again in 2-3 minutes.",
  },
...
export default function Onboarding() {
  ...
  const handleOnboard = async (event) => {
    ...
    try {
      const backendUrl =
        import.meta.env.VITE_BACKEND_URL || "http://localhost:8005";
      ...
      if (!response.ok) {
        ...
        const message =
          ...
          "Failed to onboard store. Please try again.";
  ...
  const handlePreviewWidget = () => {
    if (onboardingData?.test_url) {
      const backendUrl =
        import.meta.env.VITE_BACKEND_URL || "http://localhost:8005";
      const testUrl = `${backendUrl}${onboardingData.test_url}`;
      ...
      return (
        <div className="w-full max-w-3xl rounded-2xl border border-white/10 bg-zinc-900/70 p-8 shadow-2xl backdrop-blur">
          ...
          <p className="mt-3 text-center text-zinc-400">
            {onboardingData.products_count} products indexed • Agent trained and
            deployed
          </p>
          ...
          <div className="mt-8">
            ...
            <div className="rounded-xl border border-white/10 bg-zinc-950/90 p-4 font-mono text-xs text-zinc-300 overflow-x-auto">
              <pre className="whitespace-pre-wrap">
                {onboardingData.widget_snippet}
              </pre>
            </div>
            <p className="mt-2 text-xs text-zinc-500">
              Paste this code before the closing &lt;/body&gt; tag in your
              Shopify theme
            </p>
          </div>
          ...
          <div className="mt-6 grid grid-cols-2 gap-4 text-sm">
            <div className="rounded-lg border border-white/10 bg-white/5 p-3">
              ...
              <div className="text-white font-mono text-xs mt-1">
                {onboardingData.store_id.slice(0, 8)}...
              </div>
            </div>
            <div className="rounded-lg border border-white/10 bg-white/5 p-3">
              <div className="text-zinc-500 text-xs">Agent ID</div>
              <div className="text-white font-mono text-xs mt-1">
                {onboardingData.agent_id.slice(0, 12)}...
              </div>
            </div>
          </div>
      ...
      <div className="w-full max-w-xl rounded-2xl border border-white/10 bg-zinc-900/70 p-8 shadow-2xl backdrop-blur">
        ...
        <div className="mt-6 rounded-lg border border-white/10 bg-white/5 p-4">
          <p className="text-xs text-zinc-400">
            ...
            <br />
            We'll scrape your products, create embeddings, and deploy a voice AI
            agent trained on your catalog. This typically takes 30-60 seconds.
          </p>
        </div>
      </div>
    );

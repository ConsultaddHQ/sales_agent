...
- `www.teampop/website/` – Marketing website (React + Three.js).

Each component has its own README with detailed setup instructions.

## How it works

1. A merchant submits their store URL to `onboarding-service` which ingests
   the storefront, extracts products, embeds descriptions, and stores them
   in Supabase.
2. The `frontend` widget (embedded on the merchant site) queries
   `search-service` for relevant products during an ElevenLabs voice session.
3. `search-service` returns product cards and an LLM-generated pitch.
